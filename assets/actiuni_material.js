      ...
      fetch("../actiuni/actiune.php", {
        ...
        .then((data) => {
          if (data.success) {
            ...
            const dislikeCountEl = document.getElementById(
              `dislike-count-${id}`
            );
            ...
            const likeBtn = document.querySelector(
              `.reaction-btn.like[data-id="${id}"][data-type="${type}"]`
            );
            const dislikeBtn = document.querySelector(
              `.reaction-btn.dislike[data-id="${id}"][data-type="${type}"]`
            );
            if (!likeBtn || !dislikeBtn) {
              console.warn(
                `Like or dislike button not found for ${id} ${type}`
              );
              return;
            }
